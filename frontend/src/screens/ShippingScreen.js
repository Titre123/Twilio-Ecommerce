...
const ShippingScreen = ({ history }) => {
	...
	countryCodes.map((country) => {
		console.log(country.alpha2.toLocaleLowerCase());
	})
				...
				<Form.Group controlId='phoneNumber'>
					...
					<Col xs={4} sm={3} md={3}>
						<Select
							...
							}))} value={{ value: countryCode, label: countryCode }} onChange={(selectedOption) => setcountryCode(selectedOption.value)} isSearchable={false} />
					</Col>
					<Col xs={8} sm={9} md={9}>
						<Form.Control
						type='tel'
